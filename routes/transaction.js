...
router.post("/save", function (req, res, next) {
  let updatequery =
    "UPDATE transaction SET order_id = ?, product_id = ?, saleprice = ?, qty = ? WHERE transaction_id = " + req.body.transaction_id;
  db.query(
    updatequery,
    [
      req.body.order_id,
...
router.get("/:recordid/delete", function (req, res, next) {
  let query = "DELETE FROM transaction WHERE transaction_id = " + req.params.recordid;
  // execute query
  db.query(query, (err, result) => {
    if (err) {
      console.log(err);
